...
| Created | 2026-07-27 |
| Approved | 2026-07-27 (content through 0.3.2); 2026-07-28 (content added by 0.4.0/0.4.1 — see below) |
| Classification | Public |

This document is **Approved** (§38, Approval Status) by Denver Jacobs, Founder, acting as the interim Approval Authority for Class B (architectural) decisions under GOV-003 §3.2 while the Chief Architect role remains vacant, with the disclosed self-review/self-approval basis stated explicitly in §38 per GOV-003 §3.5. Content through version 0.3.2 was approved on 2026-07-27; the Idempotency Metadata completion added by versions 0.4.0/0.4.1 (§23.1–§23.5, invariants 42–44) was separately reviewed (Independent Architecture Review, concluding `ARCH-008 IDEMPOTENCY REVIEW REQUIRES AMENDMENT`; corrected per its two MAJOR and one MINOR finding; Independent Architecture Re-Review, concluding `ARCH-008 IDEMPOTENCY RE-REVIEW COMPLETE — READY FOR PUBLICATION`) and approved on 2026-07-28, per GOV-013 §11's requirement that amending an already-approved document requires its own evidenced Approval Authority act. This document introduces no implementation and authorizes none directly; it establishes architecture only, to be realized by a future Engineering Work Order (STD-001 §46) — EWO-001/ER-011 through EWO-013/ER-014 already realize the portion of it published to date (§36 References).
...
The current version, 0.3.2, likewise applies no technical or architectural change of any kind relative to 0.3.0/0.3.1 — it records only the formal governance disposition itself (§37, §38): the Approval Authority's decision, the completed Approval Status table, and the Approval Evidence Record.
...
| Requesting an Effect; supplying operation input; consuming the Effect outcome; deciding domain-level retry intent after restoration | **Requesting actor** (which may itself be a Provider Actor requesting a further Effect, subject to §12) |
...
- numeric retry policy (maximum attempts, backoff, jitter) and numeric timeout durations (§19, §20);
