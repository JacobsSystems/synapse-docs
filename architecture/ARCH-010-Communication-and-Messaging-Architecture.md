...
> **Status notice.** This document is **Draft**. It has not been reviewed or approved, and nothing in it should be read as operative or binding until it completes Architecture Review (GOV-013 §6.9) and Founder Approval. See §26 (Approval Status).
...
**Explicitly out of scope — acknowledged only for boundary and future-compatibility purposes, not architected here:** streaming, continuous observation (non-destructive, offset-based consumption), replay, workflow/durable execution signalling. See §21 (Future Compatibility) and §17/§22 for exactly what is deferred and why. This document does not define execution semantics for any of these, consistent with ACR-002 §3's own explicit scoping recommendation and with the task authorizing this document.
...
- **Delivery guarantees are explicit, never implicit** (ACR-002 §10/§11 invariant candidate 1; ARR-002 §5.2's own near-universal cross-system finding) — a Communication Effect's delivery guarantee is a declared property, never an ambient default.
- **Ordering, where offered, is always explicitly scoped; global ordering is never implied** (ACR-002 §10/§11 invariant candidate 3; ARR-002 §5.4/RSR-002 §4's own universal finding across every system researched).
...
**Explicitly left open — Mandatory Candidate ADR (§23):** whether authorizing access to a *dynamically-named* Topic requires a new capability dimension beyond ARCH-008 §14's existing target-binding model, or is satisfied by binding a capability to the Provider itself, with the Provider enforcing per-Topic scope internally — the identical pattern the SQLite Provider's own per-connection-handle ownership already establishes (EWO-020, `SqlError::TimerNotFound`-equivalent per-handle enforcement). ACR-002 §4/§7/§12 found this question directly blocking even the narrow, in-scope event-distribution case, and this document does not resolve it.
...
The following are explicitly, deliberately deferred by this document — named here, with the evidence-chain reasoning for each deferral, rather than left silent (ARCH-008 §4/§33's own repeatedly successful disclosed-deferral discipline, applied here):
...
| Independent Architecture Review | Not yet conducted | Not yet reviewed | — |
| Approval Authority | Not yet assigned | Not yet approved | — |

This document is **Draft**. Per GOV-013 §6.8's own Exit Criteria for Architecture Authoring, it is offered as internally consistent, faithfully transforming the design decisions already consolidated by ACR-002, introducing no new design beyond what ACR-002 authorized this document to formalize, and ready for independent Architecture Review (GOV-013 §6.9) — it is not yet operative or binding, and nothing in it should be read otherwise until that review, and a subsequent Approval Authority act, occur and are evidenced per this repository's own established convention (STD-001 §31; GOV-013 §6.16).
