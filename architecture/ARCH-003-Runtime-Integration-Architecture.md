...
**Honest reachability limitation, stated plainly:** as of the current implementation, no sequence of calls through `Runtime`'s own public API can produce a successful `suspend_actor_instance` or `restore_actor_instance` call against a genuinely live instance. This is not a defect this document resolves — it is the direct, disclosed consequence of Execution Coordinator and Lifecycle Guardian not yet being integrated (§5, §18).
...
- Successful suspend/restore reachability through the Runtime's own integrated public API (currently blocked on the first item above).
