...
2. Lifecycle Guardian: `validate_transition(&instance, ActorState::Executing)` — read-only prevalidation (EWO-005) that entry into `Executing` would be legal, performed before any handle is allocated or any Execution Coordinator state is mutated.
3. Host Adapter: `allocate_execution_handle()` — acquires a Host Execution Handle for this execution (EWO-004). On failure (currently unreachable, since allocation is unconditional), no handle exists to release.
4. Execution Coordinator: `construct_context(&instance, &message, handle)` — builds an `ExecutionContext`, embedding the handle Runtime just obtained (EWO-004); rejects with `IllegalTransition` if `instance` already has an unfinished construction in progress (ARCH-002 §12's single-execution-ownership rule, enforced by direct construction).
5. Lifecycle Guardian: `begin_execution(&instance)` — marks the instance `Executing` in Lifecycle Guardian's own tracked state (EWO-005), sequenced only after step 4 has genuinely succeeded, so a construction failure never requires any Lifecycle Guardian rollback.
6. Execution Coordinator: `dispatch(context)` — advances the tracked execution from `Constructed` to `Dispatched`; rejects if the context was never legitimately constructed or was already dispatched.
7. Execution Coordinator: `complete(context)` — removes the tracked execution entirely, freeing the instance for a subsequent execution; rejects if the context was never dispatched.
8. Lifecycle Guardian: `complete_execution(&instance)` — transitions the instance back to `Idle` (EWO-005), sequenced immediately after step 7 succeeds, before handle release or audit emission.
9. Host Adapter: `release_execution_handle(handle)` — releases the handle acquired in step 3 back to Host Adapter (EWO-004). If this fails, `execute_message` reports that failure instead of success, even though the execution itself completed.
10. Audit Emitter: `execution.completed` (ARCH-002 §18).

A failure at step 1 or step 2 causes immediate rejection before any handle is allocated. A failure at steps 4, 6, or 7 releases the handle acquired in step 3 before emitting `execution.failed` (ARCH-002 §18) and returning the triggering error — or, if that release itself fails, the release's own error is returned instead, subject to the same "a later mandatory step's failure overrides the original error" rule already established for audit emission (ADR-0015; EWO-004, "Handle Lifecycle Invariant"). A rejection at step 6 or step 7 additionally causes Lifecycle Guardian to transition the instance from `Executing` to `Failed` (EWO-005) before the handle is released; this specific path is currently reachable only through Execution Coordinator's own internal test-state seeding, never through genuine sequential public Runtime use (see §5's disclosed stale-bookkeeping limitation). Step 5 and step 8 failing after their own respective precondition has already been validated or just performed are treated as invariant violations under the current synchronous, non-interleaved implementation, not as normal, reachable failure paths (ER-005).

**Already implemented but not integrated with a wider flow:** Capability Authority is constructed as part of `TrustedCore` but is **not called anywhere in this flow**. `ExecutionContext.active_capabilities` is populated empty (Execution Coordinator has no path to Capability Authority's bindings for the instance). Lifecycle Guardian, by contrast, is now called directly by Runtime within this flow (EWO-005; steps 2, 5, 8 above) — this is no longer a gap. `ExecutionContext.host_execution_handle` (EWO-004) now carries a genuine Host-Adapter-issued value, supplied by Runtime — Execution Coordinator itself still has no path to Host Adapter (§11), and still has no path to Lifecycle Guardian (§17 invariant 5; ADR-0016).
...
- **Readiness / execution-related transitions:** ARCH-002 §15 names `Idle ⇄ Ready → Executing` and `Executing → {Idle | Failed}` among the edges relevant here. As of EWO-005, `Runtime::execute_message` drives Lifecycle Guardian's own tracked state through exactly these edges — `Idle`/`Ready → Executing` on genuine Execution Context construction, `Executing → Idle` on genuine completion, `Executing → Failed` on an authorized rejection path (§10) — for the truthful duration of the live Execution Context, and no longer. `Runtime::validate_actor_transition` continues to check proposed transitions independently of execution, unaffected by this change.
- **Suspension:** owned by Lifecycle Guardian (`Runtime::suspend_actor_instance`, verifying instance existence via Actor Host first, then delegating). Legal only from `Executing`. **Still unreachable via the Runtime's own integrated public API** — no longer because nothing marks an instance `Executing` (EWO-005 corrected that), but because, in the current fully synchronous Runtime, no call other than the one currently performing a given execution can ever observe that instance as `Executing` (ARCH-002 §12): `suspend_actor_instance` can only ever be invoked from a separate Runtime call, which by construction never overlaps with `execute_message`'s own genuinely-`Executing` span. The suspend operation itself is correctly implemented and independently exercisable using Lifecycle Guardian's own test-only state seam, scoped to that crate alone.
...
**Honest reachability limitation, now confirmed by implementation (EWO-005):** as of the current implementation, no sequence of calls through `Runtime`'s own public API can produce a successful `suspend_actor_instance` or `restore_actor_instance` call against a genuinely live instance. ARCH-002 §10/§12/§15 (v0.2.0) make explicit that an actor instance is truthfully `Executing` only for exactly as long as it owns a live Execution Context, and that within a synchronous Runtime — which this one genuinely is; no concurrent, asynchronous, or otherwise interruptible execution mechanism exists anywhere in the current implementation — no Runtime API call other than the one currently performing a given execution can ever observe that instance as `Executing`. EWO-005 has now integrated Execution Coordinator's dispatch tracking with Lifecycle Guardian's own tracked state (the item §18 previously listed), making Lifecycle Guardian's state genuinely *truthful* — a real, delivered, independently valuable improvement (§5; ER-005) — but, exactly as anticipated, this does **not**, by itself, make `suspend_actor_instance`/`restore_actor_instance` reachable against an actively-executing instance from a separate call, because no such call can ever arrive while that instance is genuinely `Executing` in a synchronous Runtime. Genuine suspend/restore reachability against an actively-executing instance still requires a real asynchronous or otherwise interruptible execution mechanism first — a prerequisite this integration phase did not provide and was not scoped to provide. See §18's remaining entry for the still-deferred consequence.
...
| `execution.failed` | Audit Emitter | `Runtime::execute_message` (via `fail_execution`) | Any rejection at steps 1, 2, 4, 6, or 7 of §10 (or a Host Adapter release failure at step 9, EWO-004) | Yes | No |
| `actor.suspended` | Audit Emitter | `Runtime::suspend_actor_instance` | Success only | Yes, but currently unreachable in an integrated flow (§12) | Yes — see §12/§18: requires a genuinely asynchronous or otherwise interruptible execution mechanism; truthful execution-state tracking is itself now complete (EWO-005) but was never sufficient alone |
...
- **Prevention of downstream side effects following rejection:** confirmed directly in §9 and §10 — a rejection at any numbered step in either flow prevents every later step in that same flow from running, with two disclosed exceptions: a rejection during Execution Context construction, dispatch, or completion (§10 steps 4, 6, 7) still causes the Host Execution Handle acquired at step 3 to be released at that point, out of its normal step-9 order, before `execution.failed` is emitted (EWO-004); and a rejection at dispatch or completion (§10 steps 6, 7) specifically also causes Lifecycle Guardian to transition the instance from `Executing` to `Failed` at that point, before that same handle release (EWO-005). Both are deliberate compensating cleanup for already-acquired resources or already-mutated state, not continued forward progress of the rejected operation — no other component's state is touched, and the operation is still reported as failed.
...
Message Gateway: validate_send_authority              | reject -> execution.failed, return Err
  | reject -> message.rejected, return Err            v
  v                                                 Lifecycle Guardian: validate_transition(Executing)  (EWO-005)
Capability Authority: validate(presented)              | reject -> execution.failed, return Err
  | reject -> message.rejected, return Err            v
  v                                                 Host Adapter: allocate_execution_handle()
Actor Host: live_instance(destination)                 | fail -> execution.failed, return Err  (EWO-004)
  | reject -> message.rejected, return Err            v
...
Message Gateway: admit(message)                         | reject -> release handle, execution.failed  (EWO-004)
  | reject -> message.rejected, return Err            v
  v                                                 Lifecycle Guardian: begin_execution(instance)  (EWO-005)
Actor Host: enqueue(instance, message)                   | reject (structurally unreachable) ->
  |                                                          release handle, execution.failed
  v                                                        v
Audit Emitter: emit "message.admitted"             Execution Coordinator: dispatch(context)
                                                        | reject -> Lifecycle Guardian: fail_execution
                                                        |           (Executing -> Failed), release
                                                        |           handle, execution.failed  (EWO-005)
                                                        v
                                                      Execution Coordinator: complete(context)
                                                        | reject -> Lifecycle Guardian: fail_execution,
                                                        |           release handle, execution.failed  (EWO-005)
                                                        v
                                                      Lifecycle Guardian: complete_execution(instance)  (EWO-005)
                                                        | reject (structurally unreachable) ->
                                                        |   release handle, execution.failed
                                                        v
                                                      Host Adapter: release_execution_handle(handle)  (EWO-004)
                                                        | fail -> execution.failed, return Err
                                                        v
...
Defined --(Actor Host: define)--> Initializing --(implied; not separately
   ...
   --> Idle <-> Ready --(Runtime::execute_message: begin_execution, EWO-005;
                          truthful for the live Execution Context's span only)--> Executing
                                                                     |
                          Lifecycle Guardian: suspend()  <-- requires Executing
                          [ UNREACHABLE today: no separate call can ever
                            arrive while genuinely Executing in a
                            synchronous Runtime (ARCH-002 §12) ]
                                                                     |
                                                                     v
                                                                Suspended
                                                                     |
                          ...
                          [ UNREACHABLE today, transitively ]
                                                                     ...
                                                                     v
                                                                   Idle
                          (Runtime::execute_message: complete_execution on
                           genuine completion; fail_execution -> Failed on
                           an authorized rejection path, not shown — EWO-005)
