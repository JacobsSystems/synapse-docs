...
related_documents:
  ...
  reviews:
    - IER-STD-031 (Independent Engineering Review of this standard's own v0.1.0, concluding `RETURN TO AUTHOR FOR REVISION` — two MAJOR findings, F01 and F02, both resolved by this revision; see §17 Revision History)
---

# STD-031 — Engineering Lifecycle Standard (Engineering Work Order Lifecycle)

> **Status notice.** This document is **Draft**. No STD-031-specific approval act has occurred. Drafting, saving, staging, committing, or pushing this document does not itself constitute its approval. See §18 (Approval Status).

> **Revision notice.** This is v0.2.0, a MAJOR revision correcting two MAJOR findings (F01, F02) an Independent Engineering Review (IER-STD-031) identified against v0.1.0. v0.1.0 restated five stages GOV-013 already fully defines — including verbatim-identical Purpose text for three of them — while explicitly claiming not to, and placed a "Publication" stage at a lifecycle position conflicting with GOV-013's own placement of that stage. Both defects are architectural, not merely editorial: this revision narrows this standard's own scope so that it no longer attempts to define any stage GOV-013 already governs, and resolves the Publication conflict by defining Publication once, as a single reusable act GOV-013 §6.16 already establishes, invoked at named points within this standard's own scope rather than redefined. See §17 for the complete disclosure.
...
- **Publication as a generic concept.** GOV-013 §6.16 defines what Publication means and requires once, generically, for any approved artifact. This standard does not redefine Publication — it identifies the specific points within the Engineering-Work-Order-execution lifecycle where GOV-013 §6.16's own generic stage is invoked, and states nothing about Publication that GOV-013 §6.16 does not already establish (§7.3).
...
- **This standard (STD-031)** is that future amendment — but only for the portion of GOV-013's lifecycle beginning after Engineering Work Order authoring (GOV-013 §6.12) is complete. For every GOV-013 stage this standard's own scope continues through (Implementation §6.13, Independent Implementation Review §6.14, Engineering Report §6.15), this standard adds only what GOV-013's own generic schema does not itself specify — required evidence fields, repository-by-repository permitted/prohibited actions, and mandatory verification/validation gates — never a restated Purpose, Permitted/Prohibited Activity, or Entry/Exit Criterion (§7.4–§7.6). For Publication (GOV-013 §6.16), this standard adds nothing to what Publication means — it identifies where, within Engineering-Work-Order execution, that one generic stage is invoked (§7.3).
...
|---|---|---|---|
| Author | Denver Jacobs (AI-assisted) | Drafted (v0.1.0), Revised (v0.2.0) | 2026-07-28 |
| Independent Engineering Review | Denver Jacobs (Founder; disclosed self-review — Chief Architect role vacant, no independent third-party reviewer available) | v0.1.0: `IER-STD-031 FAILED — RETURN TO AUTHOR FOR REVISION` (two MAJOR findings, F01 and F02, both addressed by this revision — see §17); v0.2.0 re-review: Pending | 2026-07-28 |
| Approval Authority | TBD | Pending | |

This standard is genuinely **Draft** — no Founder Approval act, and no separate evidence commit of any kind, has occurred for it. It is drafted and published to the repository as a candidate standard. This is distinct from GOV-013's own state (§3, related_documents above): GOV-013 is actually **Approved**, via a Normal-Governance evidence commit that leaves its own tracked text unedited (ADR-0011's exact-byte-identity convention). This standard carries no such evidence commit; its own `Draft` status is exactly what it appears to be. Drafting, staging, committing, or pushing this document does not itself constitute its approval.
